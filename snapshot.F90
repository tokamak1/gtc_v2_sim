subroutine snapshot
  ...
  real(wp) eright,uright,uleft,pdum,tdum,b,upara,energy,pitch,q,dela,delu,&
       ...
       eachpsi(mtheta(mpsi/2)*mzetamax),aion_inv,wt,&
...
!  tperp=0.0
     ...
     do m=1,mi 
        ...
        energy=max(1.0e-20_wp,0.5_wp*upara*upara+&
...
!     write(snapout,102)(phi00(i)/gyroradius**2,i=1,mpsi)
     ...
     dataout_r(1:mpsi,3)=phip00(1:mpsi)
     dataout_r(1:mpsi,4)=marker(1:mpsi)
     dataout_r(1:mpsi,5)=fflows(1:mpsi)
     dataout_r(1:mpsi,6)=dflows(1:mpsi)
     dataout_r(1:mpsi,7)=ftem(1:mpsi)
     dataout_r(1:mpsi,8)=dtem(1:mpsi)
