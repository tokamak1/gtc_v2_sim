...
resolve_parallel_layout() {
  ...
  if [[ "$layout" == "manual" || "$layout" == "env" ||
        ( -z "${GTC_LAYOUT+x}" && -n "$explicit_mpi" ) ]]; then
...
parallel_cores="${GTC_CORES:-10}"
layout_requested="${GTC_LAYOUT:-omp}"
mpi_np=""
omp_threads=""
...
! Defaults on this Mac use one MPI rank and ten OpenMP threads.
! Override runtime shell variables before running the script, for example:
!   MSTEP=2000 NONLINEAR=0.0 SPECTRUM_MODE=1 ./run_itg_cyclone_base_nonlinear_zonal_c.sh
!
! Parameter notes:
