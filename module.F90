...
module global_parameters
  ...
  integer mi,mimax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,msnap,mstep,mstepall,stdout,mype,numberpe,&
       mode00,nbound,irun,iload,irk,idiag,mtdiag,idiag1,idiag2,&
       ntracer1,nparam,rng_control,nspec        
...
module particle_array
  use precision
  integer,dimension(:),allocatable :: kzion
  integer,dimension(:,:),allocatable :: jtion0,jtion1
  real(wp),dimension(:),allocatable :: wzion
  real(wp),dimension(:,:),allocatable :: wpion,wtion0,wtion1    
  real(wp),dimension(:,:),allocatable :: zion,zion0
...
module particle_tracking
  ...
  real(wp),dimension(:,:),allocatable :: ptrackedi    
...
module field_array
  ...
  real(wp),dimension(:),allocatable :: phi00,phip00,rtemi,rden,qtinv,&
       pmarki,zonali,gradt
  real(wp),dimension(:,:),allocatable :: phi,densityi,markeri,&
...
module diagnosis_array
  use precision
  integer,parameter :: mflux=5,num_mode=13,m_poloidal=9
  integer nmode(num_mode),mmode(num_mode)
  real(wp) efluxi,pfluxi,ddeni,dflowi,&
       entropyi,efield,eradial,particles_energy(2),eflux(mflux),&
       rmarker(mflux),amp_mode(2,num_mode,2)
  real(wp),dimension(:),allocatable :: hfluxpsi,rdtemi,pfluxpsi
