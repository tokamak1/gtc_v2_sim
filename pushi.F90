subroutine pushi
  ...
  integer i,im,ip,j,jt,k,kz,kk,m,ii,j11,j10,j01,j00,jtheta,num,ierror,&
       larmor,ij,ipjt,irsmooth,iir(mi)
...
! an error on the Linux platform. We thus make sure that the 2 buffers are different.
       dtemtmp=0.0
       ...
       dmark=dmarktmp/max(1.0_wp,ddentmp) !radial marker flux
       do i=1,irsmooth
          rdum=dmark(1)
          tdum=dmark(mpsi-1)
...
! radial location in diagnostic bin
       ...
       vdrenergy=rinv*wpi(1,m)*(wpi(2,m)-1.5*aion*vthi*vthi*rtemi(ii))*zion0(5,m)

! radial profile of heat flux
       hfluxpsi(ii)=hfluxpsi(ii)+vdrenergy ! energy flux profile       
       dden(ii)=dden(ii)+1.0
       d_efluxi=d_efluxi+vdrenergy
       d_pfluxi=d_pfluxi+rinv*wpi(1,m)*zion0(5,m)
    ...
    if(mype==0)then
         open(345,file="ddeni.dat",status="replace")
         write(345,102)dden(1:mpsi)
         close(345)
    102 format(e12.6)
    endif
